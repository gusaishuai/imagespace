...
//import com.imagespace.sql.model.XSqlPagination;
...
//            SqlKeyWord sqlKeyWordFirst = null;
...
//                        } else if (sqlKeyWord.isNeedWhere() && !SqlKeyWord.WHERE.contain(sqlEach)) {
...
//                }
//            }
//            if (SqlKeyWord.SELECT.start(sql) || SqlKeyWord.SHOW.start(sql)) {
//                // 查询
//                return jdbcTemplate.queryForList(StringUtils.trim(sqlEachArray[0]));
//            } else if (SqlKeyWord.INSERT.start(sql) || SqlKeyWord.UPDATE.start(sql)
//                    || SqlKeyWord.DELETE.start(sql)) {
