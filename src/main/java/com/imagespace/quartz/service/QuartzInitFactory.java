                ...
                Optional.ofNullable(SpringContext.getBean(beanName))
                    ...
                    try {
                        if (!Objects.equals(Void.TYPE, method.getReturnType())) {
                            throw new IllegalArgumentException("定时任务方法返回类型必须为void");
                        }
                        if (method.getParameterCount() != 0) {
                            throw new IllegalArgumentException("定时任务方法的入参必须为空");
                        }
                        Date startDate;
                        ...
                        log.info("定时任务：{} 初始化完毕", quartzName);
                    } catch (IllegalArgumentException e) {
                        log.error("定时任务：{} 初始化失败：{}", quartzName, e.getMessage());
                    } catch (Exception e) {
                        log.error("定时任务：{} 初始化失败", quartzName, e);
                    }
                }));
            });
            //项目中有对应的定时任务才启动
